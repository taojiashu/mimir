...
CONFIG="configs/token_ref_1ref_step100000.json"
NGRAM=13
...
LOG_DIR="logs/token_ref_1ref_step100000"
mkdir -p "${LOG_DIR}"

for model in "${MODELS[@]}"; do
  BASE_MODEL="EleutherAI/${model}-deduped"
  for subset in "${SUBSETS[@]}"; do
    SPECIFIC_SOURCE="${subset}_ngram_${NGRAM}<0.8_truncated"
    echo "[token_ref_1ref] Running base_model=${BASE_MODEL} subset=${subset}"
